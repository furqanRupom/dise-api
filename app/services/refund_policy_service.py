...
class RefundPolicyService:
    ...
    def get_tier(self, tier_id: uuid.UUID) -> RefundPolicyTier:
        """Get a refund policy tier by ID."""
        refund_tier = self.db.execute(
            select(RefundPolicyTier).where(
    ...
    def create_tier(
        self,
        payload: RefundPolicyTierCreate,
    ) -> RefundPolicyTier:
        """Create a new refund policy tier."""
        existing_tier = self.db.execute(
            select(RefundPolicyTier).where(
    ...
    def update_tier(
        self,
        tier_id: uuid.UUID,
        payload: RefundPolicyTierUpdate,
    ) -> RefundPolicyTier:
        """Update a refund policy tier."""
        refund_tier = self.get_tier(tier_id)
    ...
    def compute_refund_percentage(
        ...
        Tiers are step thresholds: the highest hours_before_pickup value
        that the actual hours_until_pickup still satisfies wins.
        """
